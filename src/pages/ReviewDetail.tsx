...
const reviewsData: Record<string, ReviewData> = {
  "tt35499025-review": {
    ...
    date: "March 12, 2026",
    content: [
      { type: "p", text: "大自然对我们说了什么？东华在寺庙谈论我们如何用身体感受世界，起身离开，摄影机呈现了一个向上抬起的动作，我们看着让东华在后文想起埋葬岳父的母亲的那棵树，画面在这停下若干秒，树叶之间透过阳光。洪常秀好像在对我们发出邀请：就像重复两次“我不害怕”的蹩脚的诗，洪常秀以这个可感知的摄影机动作诚挚地露出了马脚。然后树变成了电影，我们留下了印象，不自觉地想着诗人与女友“用身体感受世界”的对话，心里好像也产生了一首语无伦次的诗。"},
      { type: "p", text: "如何在物件中找到电影，让它们变得有趣？洪常秀讲故事有自己的节奏，进行将几位角色进行排列组合的游戏，家庭成员各自按照一些组别对话，于是就出现了隐含幽默的重复。若干次抽烟，父亲递烟，在车上闻到烟味，他们相约抽烟，以及吃完午饭后姐姐尚未出场，东华又点了根烟；若干次提及俊熙，东华与父亲和姐姐分别讲出对女友的看法，对应俊熙在场以及不在场时，东华的不同姿态。"},
      { type: "p", text: "幽默往往发生在不按语言提示的模式进行的场景，他们相约去抽烟，他们路过狗，路过鸡，聊了很多，却没有在抽烟。鸡几乎变成这部电影的全部：第一次路过鸡，东华说这些鸡“长得好看”，而权海骁则说这些鸡会被做成清炖（Baeksuk），一个错位；他们正经地在路边聊天，背景声是鸡在旁边此起彼伏的鸣叫，声音的交叠产生影片最幽默的时刻，这是另一个错位。然后东华与姐妹俩吃饭，提及晚饭的Baeksuk鸡，切换到另一个场景，父亲在抓鸡，鸡群躲开，我们不知道哪只鸡成为了晚饭餐桌上的佳肴。这些错位使电影存在，电影成为洪常秀的玩笑。"},
      { type: "p", text: "鸡的存在好像到最后餐桌的场景才彻底失去了声音。狗呢？他们路过狗，狗没有出现，只是不断吠叫，和鸡同构。当河成国半夜爬到山上看月亮，走了捷径，又在下坡时跌倒，那个“有人经过就会吠叫”的狗却隐身，洪巧妙地给出了一次对比，没有发出任何声音。"},
      { type: "p", text: "东华被认可前走入房子，姐姐在弹古琴；东华不被认可后走出房子，父亲在弹吉他。而掌握话语权的却是始终在付出的母亲，她的身份和东华一样，是位知名的诗人。这已经是近乎荒谬的结构，在这个结构里，我们看不见诗，只能看到电影。而作诗的动作只在两个“过于电影”的时刻发生：抽烟过半，父亲去拿酒，镜头突然从只能容纳两人上半身的中景拉至远景，留下东华坐在长椅，在本子上写作一些东西，然后父亲带酒进入画面，镜头缩进到更近的中景；第二次是餐桌的场景，又是在喝酒，所有人紧盯着东华尴尬作诗，同样很幽默。"},
      { type: "p", text: "虚焦就像是洪所开的玩笑。当东华戴上眼镜，我们看到了河成国，以不那么常见的形象在洪常秀电影中出现。当东华取下眼镜，画面和东华的视线变得一样模糊，分辨率降低，山上的树与草都融为一片绿色，像是《在水中》的游戏。"},
    ],
  },
  "yes-review": {
    id: "yes-review",
